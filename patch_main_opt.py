...
# Let's find exactly the line: "setTasks(prev => [data.data, ...prev]);"
# But wait, there are multiple `data = await res.json()` in this file!
# But the `fallbackTask` logic is unique.
...
if start_str not in content:
    start_str = start_str.replace('Tạo', 'T?o') # if encoding gets messed up again
...
if start_idx != -1 and end_idx != -1 and start_idx < end_idx:
    ...
    with open('agent/rules/stitch_smart_ai_task_management_system/src/App.jsx', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Patched successfully via precise index.")
else:
