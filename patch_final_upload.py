...
# First check if multer is already imported
if "const multer =" not in text:
    multer_import_code = """
...
"""
    # Replace the stub with multer config + new endpoint
...
app.post('/api/rag/upload', authenticateUser, checkAdmin, (req, res, next) => {
    ...
    text = re.sub(stub_pattern, new_upload_endpoint, text)
