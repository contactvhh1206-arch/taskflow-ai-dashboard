...
export default function AdminConfigPanel({ showToast, tasks, setTasks, setTaskComments, user }) {
      ...
      const handleLearnFromChat = async (chatLogData) => {
          ...
          try {
              ...
              const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://taskflow-ai-dashboard.onrender.com'}/api/rag/learn-from-chat`, {
                  method: 'POST',
                  headers: {
      ...
      const handleResetSystem = () => {
         ...
         setTimeout(async () => {
            try {
               ...
               try {
                   const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://taskflow-ai-dashboard.onrender.com'}/api/system/reset`, {
                       method: 'DELETE',
                       headers: {
                           'Content-Type': 'application/json',
      ...
      const handleUpdateFacility = async (e) => {
        ...
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://taskflow-ai-dashboard.onrender.com'}/api/facilities/${editingFac.id}`, {
                method: 'PUT',
                headers: { 
                  'Content-Type': 'application/json',
      ...
      const handleDeleteFacility = async () => {
        ...
        try {
          const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://taskflow-ai-dashboard.onrender.com'}/api/facilities/${deletingFac.id}`, { method: 'DELETE' });
          if (res.ok) {
            await fetchFacilities();
      ...
      const handleDeleteUser = async () => {
        ...
        try {
          const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://taskflow-ai-dashboard.onrender.com'}/api/users/${deletingUser.id}`, { method: 'DELETE' });
          if (res.ok) {
            await fetchUsers();
      ...
      const handleUpdateUser = async (e) => {
        ...
        try {
            const token = localStorage.getItem('taskflow_token');
            const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://taskflow-ai-dashboard.onrender.com'}/api/users/${editingUser.id}`, {
                method: 'PUT', headers: { 
                  'Content-Type': 'application/json',
                  'Authorization': token ? `Bearer ${token}` : '',
                  'x-user-role': user?.role || '',
      ...
      const toggleUserActive = async (userId) => {
        ...
        try {
            const token = localStorage.getItem('taskflow_token');
            const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://taskflow-ai-dashboard.onrender.com'}/api/users/${userId}`, {
                method: 'PUT', headers: { 
                  'Content-Type': 'application/json',
                  'Authorization': token ? `Bearer ${token}` : '',
                  'x-user-role': user?.role || '',
