             ...
             <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
                <table className="w-full text-sm text-left border border-gray-200 dark:border-gray-800">
                  <thead className="bg-[#e2eaf5] dark:bg-blue-900/30">
                    <tr>
                      <th className="px-4 py-3 border-b border-gray-200 dark:border-gray-800 font-bold text-gray-700 dark:text-gray-300">Ngày</th>
                      {facilities.map(f => (
                         <th key={f.id} className="px-4 py-3 border-b border-gray-200 dark:border-gray-800 font-bold text-gray-700 dark:text-gray-300 text-right">{f.name}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {data.map((row, i) => (
                      <tr key={i} className="border-b border-gray-100 dark:border-gray-800 bg-[#f9fafb] dark:bg-[#1a1a1a]">
                        ...
                        {facilities.map(f => {
                           ...
                           return (
                             <td key={f.id} className="px-4 py-3 text-right text-gray-900 dark:text-gray-100">{new Intl.NumberFormat('vi-VN').format(val)}</td>
                           );
                        })}
                      </tr>
                    ))}
                  ...
                  <tfoot className="bg-[#c2d0e7] dark:bg-blue-900/60 font-bold">
                    <tr>
                      <td className="px-4 py-3 text-gray-900 dark:text-white uppercase">Tổng {data.length} Ngày</td>
                      {facilities.map(f => {
                         const facNameShort = f.name.replace('DUBAI ', '');
                         const total = data.reduce((acc, row) => acc + Number(row.revenues[f.name] || row.revenues[facNameShort] || 0), 0);
                         return (
                           <td key={f.id} className="px-4 py-3 text-right text-teal-700 dark:text-teal-400">{new Intl.NumberFormat('vi-VN').format(total)}</td>
                         );
                      })}
                    </tr>
                  </tfoot>
