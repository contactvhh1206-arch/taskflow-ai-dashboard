...
const ragController = {
    uploadAndVectorizeDocument: async (req, res) => {
        ...
        try {
            ...
            res.status(500).json({ success: false, error: "Dịch vụ AI gián đoạn: " + error.message });
        } finally {
            client.release();
        }
    },
...
const startIndex = content.indexOf("app.post('/api/rag/upload'");
if (startIndex !== -1) {
    const endStr = "return res.json({";
    const endIndex = content.indexOf(endStr, startIndex);
    if (endIndex !== -1) {
        const finalEnd = content.indexOf("});", endIndex) + 3;
        const blockToRemove = content.substring(startIndex, finalEnd);
        // Find the catch block of the old route which extends further
        const finalCatchEnd = content.indexOf("});", finalEnd) + 3;
        
        content = content.replace(content.substring(startIndex, finalCatchEnd), newCode);
